...
namespace DuplicateFinder.Core.Integration.Tests
{
  ...
  public class When_all_seen_files_are_pruned : HistorySpecs
  {
    ...
    Establish context = () =>
    {
      ...
      var run1 =
        parser.Parse(
                     CommandLine.EnumerableExtensions.Args((@"--name --history " + History +
                                                            @" HashCodeHistory_Prune\Run1_FileExists")));
      run1.Execute();

      var run2 =
        parser.Parse(
                     CommandLine.EnumerableExtensions.Args((@"--name --history " + History +
                                                            @" HashCodeHistory_Prune\Run2_FileWasDeleted")));
      run2.Execute();

      Prune =
        parser.Parse(
                     CommandLine.EnumerableExtensions.Args((@"--prune --name --history " + History +
                                                            @" HashCodeHistory_Prune")));
    };
  ...
  public class When_files_are_pruned_with_whatif : HistorySpecs
  {
    ...
    Establish context = () =>
    {
      ...
      Prune =
        parser.Parse(
                     CommandLine.EnumerableExtensions.Args((@"--prune --whatif --name --history " + History +
                                                            @" HashCodeHistory_Prune\Run1_FileExists")));
    };
  ...
  public class When_all_seen_files_are_pruned_and_later_are_recreated : HistorySpecs
  {
    ...
    Because of = () =>
    {
      ...
      Parser
        .Parse(
               CommandLine.EnumerableExtensions.Args((@"--name --history " + History +
                                                      @" HashCodeHistory_Prune\Run4_FileIsRecreated")))
        .Execute();
    };
  ...
  public class When_a_subset_of_seen_files_is_pruned : HistorySpecs
  {
    ...
    Establish context = () =>
    {
      ...
      var run1 =
        parser.Parse(
                     CommandLine.EnumerableExtensions.Args((@"--name --history " + History +
                                                            @" HashCodeHistory_PruneSubset\Run1_FileExists")));
      run1.Execute();

      var run2 =
        parser.Parse(
                     CommandLine.EnumerableExtensions.Args((@"--name --history " + History +
                                                            @" HashCodeHistory_PruneSubset\Run2_FileWasDeleted")));
      run2.Execute();
  ...
  public class When_a_subset_of_seen_files_is_pruned_and_later_is_recreated : HistorySpecs
  {
    ...
    Because of = () =>
    {
      ...
      Parser
        .Parse(
               CommandLine.EnumerableExtensions.Args((@"--name --history " + History +
                                                      @" HashCodeHistory_PruneSubset\Run3_FileIsRecreated")))
        .Execute();
    };
